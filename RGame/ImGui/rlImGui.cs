...
namespace RGame
{
    public static class rlImGui
    {
        ...
        private static void EndInitImGui()
        {
            ...
            io.Fonts.AddFontFromFileTTF("Assets/fonts/OpenSans/static/OpenSans/OpenSans-Regular.ttf", 18.0f);
        ...
        private static void NewFrame()
        {
            ...
            var imgCursor = ImGui.GetMouseCursor();
            if (imgCursor == CurrentMouseCursor && !io.MouseDrawCursor) return;
            CurrentMouseCursor = imgCursor;
            if (io.MouseDrawCursor || imgCursor == ImGuiMouseCursor.None)
            {
                Raylib.HideCursor();
            }
            else
            {
                ...
                if ((io.ConfigFlags & ImGuiConfigFlags.NoMouseCursorChange) != 0) return;
                Raylib.SetMouseCursor(MouseCursorMap != null && !MouseCursorMap.ContainsKey(imgCursor)
                    ? MouseCursor.MOUSE_CURSOR_DEFAULT
                    : MouseCursorMap![imgCursor]);
            }
        }
